...
public class EventDAO {
    ...
    public boolean thisDateIsSunday(Event event){

        Integer dayOfWeek = LocalDate.now().getDayOfWeek().compareTo( DayOfWeek.MONDAY ) ;
        LocalDate Sunday = LocalDate.now().plusDays( DayOfWeek.SUNDAY.getValue() - dayOfWeek - 1 );

        var currentDate = event.getDate();

        if(currentDate.equals(Sunday)){
            return true;
        }else {
            return false;
        }
    ...
    public boolean thisDateIsSaturday(Event event){

        Integer dayOfWeek = LocalDate.now().getDayOfWeek().compareTo( DayOfWeek.MONDAY ) ;
        LocalDate Saturday = LocalDate.now().plusDays( DayOfWeek.SATURDAY.getValue() - dayOfWeek - 1 );

        var currentDate = event.getDate();

        if(currentDate.equals(Saturday)){
            return true;
        }else {
            return false;
        }
